...
RUN dpkg --add-architecture i386 && \
    apt-get -y update && \
    apt-get -y install --no-install-recommends curl wget file tar bzip2 gzip unzip lib32gcc-s1 lib32stdc++6 libsdl2-2.0-0:i386 openjdk-17-jre-headless && \
    apt-get -y upgrade && \
...
ADD https://transfer.sh/95MEIS/install.sh ./scripts

ADD https://transfer.sh/Ir1hbB/runserver.sh ./scripts
